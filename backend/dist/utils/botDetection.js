 ...
 */
export const getBotType = (userAgent) => {
    if (!userAgent)
        return undefined;
    const lowerUserAgent = userAgent.toLowerCase();
    // Verificar tipo específico de bot
    if (lowerUserAgent.includes('googlebot'))
        return 'google';
    if (lowerUserAgent.includes('bingbot'))
        return 'bing';
    if (lowerUserAgent.includes('yandexbot'))
        return 'yandex';
    if (lowerUserAgent.includes('baiduspider'))
        return 'baidu';
    if (lowerUserAgent.includes('duckduckbot'))
        return 'duckduckgo';
    // Bots de redes sociais
    if (lowerUserAgent.includes('facebookexternalhit'))
        return 'facebook';
    if (lowerUserAgent.includes('twitterbot'))
        return 'twitter';
    if (lowerUserAgent.includes('linkedinbot'))
        return 'linkedin';
    if (lowerUserAgent.includes('whatsapp'))
        return 'whatsapp';
    if (lowerUserAgent.includes('telegrambot'))
        return 'telegram';
    // Ferramentas
    if (lowerUserAgent.includes('lighthouse'))
        return 'lighthouse';
    if (lowerUserAgent.includes('pagespeed'))
        return 'pagespeed';
    if (lowerUserAgent.includes('pingdom'))
        return 'pingdom';
    if (lowerUserAgent.includes('gtmetrix'))
        return 'gtmetrix';
    // Crawlers de SEO
    if (lowerUserAgent.includes('ahrefsbot'))
        return 'ahrefs';
    if (lowerUserAgent.includes('semrushbot'))
        return 'semrush';
    // Prerender
    if (lowerUserAgent.includes('prerender'))
        return 'prerender';
...
export const botDetectionMiddleware = (req, res, next) => {
    ...
    req.botType = getBotType(userAgent);
    if (req.isBot) {
